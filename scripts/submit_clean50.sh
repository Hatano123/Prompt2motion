...
if [[ ! -s "$archive" || ! -s "$validation_manifest" || ! -s "$test_manifest" ]]; then
  ...
  data_dependency="afterok:$data_job"
...
for spec in 'single_top:top' 'single_side:side' 'multi:top,side'; do
  ...
  for TRAIN_SEED in 0 1 2; do
    ...
    elif [[ -n "$data_dependency" ]]; then
      train_job="$(qsub -W depend="$data_dependency" -v MODEL_NAME,CAMERA_NAMES,TRAIN_SEED,BATCH_SIZE scripts/run_clean50_train.pbs)"
    ...
    if [[ -e "$result_dir/eval_test/.complete" ]]; then
      ...
      test_job="$(qsub -W depend="afterok:$train_job" -v MODEL_NAME,CAMERA_NAMES,TRAIN_SEED,BATCH_SIZE scripts/run_clean50_test.pbs)"
      echo "$MODEL_NAME seed=$TRAIN_SEED test=$test_job"
    else
      test_job="$(qsub -v MODEL_NAME,CAMERA_NAMES,TRAIN_SEED,BATCH_SIZE scripts/run_clean50_test.pbs)"
      echo "$MODEL_NAME seed=$TRAIN_SEED test=$test_job"
    fi
  done
done
